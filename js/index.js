...
window.onload = function () {
  //Creo el array de usuarios de esta manera para que no se vuelva a generar vacío cada vez que se entra en la página
  if (localStorage.getItem("hasCodeRunBefore") === null) {
    let users = [];
    localStorage.setItem("users", JSON.stringify(users));
    let activeUser; //Creo una variable para saber qué usuario está logueado
    localStorage.setItem("activeUser", JSON.stringify(activeUser));
    let movieIDLS; //Creo una variable para almacenar el ID de una película y acceder a sus detalles
...
function checkLoginUser(formUsername, formPassword) {
  ...
  if (User.isRegistered(formUsername)) {
    //Si el usuario está registrado, verificamos la contraseña
    if (User.verifyPassword(formUsername, formPassword)) {
      let userIndex = users.findIndex(
        (user) => user._username === formUsername,
      );
      ...
      return alert(
        `Contraseña incorrecta. Por favor, introduce la contraseña nuevamente.`,
      ); //Si la contraseña no es OK, avisamos al usuario
    }
  } else {
    return alert(
      `Este usuario no está registrado. Por favor, haz click en "Nuevo usuario" y regístrate.`,
    ); //Si el usuario no existe, damos instrucciones para que se registre
  }
}
