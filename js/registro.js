...
function isPostCodeValid() {
  let isPostCode = cities.some(
    (city) =>
      city.postalCode.slice(0, 2) ==
      document.getElementById("postalCode").value.slice(0, 2),
  ); //Busco si dentro del array de ciudades hay algún objeto cuyas dos primeras cifras sean iguales a las dos primeras cifras del código postal ingresado por el usuario
  let isPostCodeValid;
  if (document.getElementById("postalCode").value.length > 1) {
    //Me aseguro que no esté vacío
    if (isPostCode && document.getElementById("postalCode").value.length < 6) {
      //Si obtenemos que la condición anterior se cumple en el array, y que el CP tiene menos de 6 dígitos, buscaré el objeto del array cuyo código postal tenga los dos primeros dígitos que el código postal ingresado por el usuario
      ...
      alert(
        "El código postal introducido no es válido. Por favor, introduce un código postal correcto",
      ); //Si el código postal es de más de 6 dígitos o no está en el array de ciudades, le aviso al usuario
...
document.getElementById("username").addEventListener("blur", () => {
  if (User.isRegistered(document.getElementById("username").value)) {
    alert(
      'El usuario ya existe. Por favor, ingresa un usuario diferente o haz click en "Volver al Login" para iniciar sesión',
    ); //Avisa si se ha ingresado un usuario que ya existe
  }
});
...
document.getElementById("city").addEventListener("change", () => {
  let cityObj = cities.find(
    (city) => city.name == document.getElementById("city").value,
  );
...
document
  ...
  .addEventListener("keyup", isPostCodeValid);

/*Autorrellenar el correo con @gmail.com una vez llegados al @. Cada vez que se deja de presionar una tecla, compruebo si el campo de texto incluye un @, si no es el caso
agregamos a nuestra string el dominio deseado, siempre y cuando no se haya agregado ya*/
document.getElementById("email").addEventListener("keyup", () => {
  if (
    document.getElementById("email").value.includes("@") &&
    !document.getElementById("email").value.includes("gmail.com")
  ) {
...
document
  ...
  .addEventListener("blur", () =>
    User.isEmailValid(document.getElementById("email").value),
  );
...
document
  ...
  .addEventListener("change", () =>
    User.isPasswordValid(document.getElementById("password").value),
  );
...
document.getElementById("registerForm").addEventListener("submit", (event) => {
  ...
  if (User.isRegistered(document.getElementById("username").value)) {
    //Antes de enviar la información comprobamos si el usuario ya existe, en ese caso no enviamos
    alert(
      'El usuario ya existe. Por favor, ingresa un usuario diferente o haz click en "Volver al Login" para iniciar sesión',
    );
  } else if (
    !User.isRegistered(document.getElementById("username").value) &&
    isPostCodeValid() &&
    User.isEmailValid(document.getElementById("email").value) &&
    User.isPasswordValid(document.getElementById("password").value)
  ) {
    //Si el usuario no existe, y además los campos de código postal, email y contraseña son válidos
    const newUser = new User(
      ...
      watched,
      pending,
    ); // Con los valores de la variables del contenido del formulario, creamos un nuevo objeto user con el constructor
...
document
  .getElementById("returnToLoginButton")
  .addEventListener(
    "click",
